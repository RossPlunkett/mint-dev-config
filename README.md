...
Start from a normal Mint 22.3 installation with your user account, internet access, and Git:
...
After installation, log out or reboot so the Docker group and desktop-session changes take effect. Then complete [AUTH_CHECKLIST.md](AUTH_CHECKLIST.md) and run:
...
- NVM and Node 22; Homebrew does not own Node
...
This repository never copies monitor layouts, display hotplug rules, machine identifiers, credentials, tokens, login state, Android signing keys, or T3 application state. Git identity is also left to each machine.
